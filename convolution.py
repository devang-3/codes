...
def add_padding(matrix: np.ndarray, 
    ...
    n, m = matrix.shape  # dimension of the matrix
    r, c = padding   # number of rows and columns to be padded

    padded_matrix = np.zeros((n + 2 * r, m + 2 * c))  # create a new matrix with the new shape and fill it with zeros
    padded_matrix[r:r + n, c:c + m] = matrix   # fill the new matrix with the original matrix values
...
def _check_params(matrix, kernel, stride, dilation, padding):
    ...
    matrix_to_kernel_is_correct = (
        n_p >= effective_kernel_shape[0] and m_p >= effective_kernel_shape[1]
    )
...
def apply_filter_to_image(image: np.ndarray,
    ...
    Args:
        image (np.ndarray): 2D grayscale image or 3D HWC image.
        kernel (List[List[float]]): 2D odd-shaped matrix (e.g. 3x3, 5x5, 13x9, etc.).

    Returns:
        np.ndarray: image after applying kernel. Integer images are clipped to
            their dtype range; floating-point images retain floating-point values.
    """
    if image.ndim == 2:
        filtered = conv2d(image, kernel, padding=(len(kernel)//2, len(kernel[0])//2))
    elif image.ndim == 3:
        padding = (len(kernel)//2, len(kernel[0])//2)
        filtered = np.stack([
            conv2d(image[:, :, channel], kernel, padding=padding)
            for channel in range(image.shape[2])
        ], axis=2)
    else:
        raise ValueError('Image must be a 2D grayscale or 3D HWC array.')
